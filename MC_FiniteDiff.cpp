...
vector<float> MC_FiniteDiff::price(string option_type, int N){
...
vector<float> MC_FiniteDiff::delta(string option_type, int N) {
	float dS0 = 0.0001;
    ...
    for (int i = 0; i < N; i++) {
        ...
        float actualised_payoff1 = exp(-r * T) * payoff(option_type, St2, K);
		d_sum1 += actualised_payoff1;
        d_sum2 += actualised_payoff1;
...
vector<float> MC_FiniteDiff::gamma(string option_type, int N) {
    float dS0 = 0.0001;
...
vector<float> MC_FiniteDiff::vega(string option_type, int N) {
	float dsig = 0.0001;
